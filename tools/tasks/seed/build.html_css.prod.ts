...
function processComponentScss() {
  return gulp.src(join(APP_SRC, '**', '*.scss'))
    .pipe(plugins.cached('process-component-scss'))
...
function getExternalScssStream() {
  return gulp.src(getExternalScss())
    .pipe(plugins.cached('process-external-scss'))
    .pipe(plugins.progeny())
    .pipe(plugins.sass({includePaths: ['./node_modules/']}).on('error', plugins.sass.logError));
}
 ...
 */
export = () => merge(processComponentStylesheets(), processExternalStylesheets());
